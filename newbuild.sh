...
echo >> "$CHANGELOG_FILE"
git log "$LAST_TAG"..HEAD --oneline --format=oneline | sed 's/^.\{13\}//g' | sed 's/^/* /' >> "$CHANGELOG_FILE"
}
...
function menu()  {
	...
	echo -e "7: Generate FKM Changelog"
	echo -e "X: Exit to Shell"
	echo -e ""
	...
	case $choice in
		1) echo -e "$DIVIDER"
	 	...
	 	6) toggle_prefs
	 	   ;;
	 	7) echo -e "$DIVIDER"
	 	   echo -e "$red Generating changelog"
	 	   echo -e "$DIVIDER"
	 	   generate_changelog
	 	   menu
	 	   ;;
	 	X) echo -e "$red Exiting"
	 	   clear
	 	   ;;
	 	1010) debug_menu
	 	   ;; 
